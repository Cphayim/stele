...
describe('[Mutation.bookTrips]', () => {
  const {
    bookTrips
  } = mockContext.dataSources.userAPI;
  const {
    getLaunchesByIds
  } = mockContext.dataSources.launchAPI;
  it('returns true if booking succeeds', async () => {
    ...
    const res = await resolvers.Mutation.bookTrips(null, {
      launchIds: [123]
    }, mockContext);
    expect(res).toEqual({
      launches: [{
    ...
    expect(bookTrips).toBeCalledWith({
      launchIds: [123]
    });
  });
  it('returns false if booking fails', async () => {
    bookTrips.mockReturnValueOnce([]); // check the resolver response
...
describe('[Mutation.cancelTrip]', () => {
  ...
  it('returns true if cancelling succeeds', async () => {
    ...
    const res = await resolvers.Mutation.cancelTrip(null, {
      launchId: 123
    }, mockContext);
    expect(res).toEqual({
      success: true,
    ...
    expect(cancelTrip).toBeCalledWith({
      launchId: 123
    });
  });
  it('returns false if cancelling fails', async () => {
    cancelTrip.mockReturnValueOnce(false); // check the resolver response
...
describe('[Mutation.login]', () => {
  ...
  it('returns base64 encoded email if successful', async () => {
    ...
    const base64Email = new Buffer(mockContext.user.email).toString('base64'); // check the resolver response

    const res = await resolvers.Mutation.login(null, args, mockContext);
    expect(res).toEqual('YUBhLmE='); // check if the dataSource was called with correct args
  ...
  it('returns nothing if login fails', async () => {
    ...
    findOrCreateUser.mockReturnValueOnce(false); // check the resolver response
