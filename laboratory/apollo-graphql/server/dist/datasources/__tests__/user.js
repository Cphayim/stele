...
describe('[UserAPI.bookTrip]', () => {
  it('calls store creator and returns result', async () => {
    mockStore.trips.findOrCreate.mockReturnValueOnce([{
      get: () => 'heya'
    }]); // check the result of the fn

    const res = await ds.bookTrip({
      launchId: 1
    });
    expect(res).toBeTruthy(); // make sure store is called properly
...
describe('[UserAPI.cancelTrip]', () => {
  it('calls store destroy and returns result', async () => {
    ...
    mockStore.trips.destroy.mockReturnValueOnce('heya'); // check the result of the fn

    const res = await ds.cancelTrip(args);
    expect(res).toEqual(true); // make sure store is called properly

    expect(mockStore.trips.destroy).toBeCalledWith({
      where: args
    });
  });
});
describe('[UserAPI.getLaunchIdsByUser]', () => {
  it('looks up launches by user', async () => {
    ...
    mockStore.trips.findAll.mockReturnValueOnce(launches); // check the result of the fn

    const res = await ds.getLaunchIdsByUser(args);
    expect(res).toEqual([1, 2]); // make sure store is called properly
