...
describe('[Query.launches]', () => {
  ...
  it('respects pageSize arg', async () => {
    ...
    const res = await resolvers.Query.launches(null, {
      pageSize: 1
    }, mockContext);
    expect(res).toEqual({
      cursor: 'bar',
...
describe('[Query.launch]', () => {
  ...
  it('calls lookup from launch api', async () => {
    ...
    const res = await resolvers.Query.launch(null, {
      id: 999
    }, mockContext);
    expect(res).toEqual({
      id: 999
    ...
    expect(getLaunchById).toBeCalledWith({
      launchId: 999
    });
  });
});
describe('[Query.me]', () => {
  ...
  it('returns user from userAPI', async () => {
    ...
    findOrCreateUser.mockReturnValueOnce({
      id: 999
